...
class Clinic(models.Model):
    ...
    Health_Facility_License = models.CharField(max_length=50, null=True, blank=True)
...
class Doctor(models.Model):
    ...
    clinic = models.ForeignKey(Clinic, on_delete=models.SET_NULL, related_name='linked_doctors', null=True, blank=True)
...
class ClinicInvitation(models.Model):
    ...
    clinic = models.ForeignKey(Clinic, on_delete=models.CASCADE, related_name='sent_invitations')
    doctor = models.ForeignKey(Doctor, on_delete=models.CASCADE, related_name='received_invitations')
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='pending')
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Invite from {self.clinic.user.first_name} to {self.doctor.user.first_name}"
